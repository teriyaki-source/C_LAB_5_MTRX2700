...
SerialPort USART1_PORT = {USART1,
		...
		0xA00,
		0xF00,
		0x770000,  // for USART1 PC4 and 4, this is in the AFR low register
...
void SerialReceiveString(SerialPort *serial_port) {
	uint8_t buffer[10] = {0};
	uint8_t termination = 13;
	uint8_t last_char = 0;
	int i = 0;

	while (last_char != termination){
	...
	SerialOutputString(buffer, serial_port);
}
